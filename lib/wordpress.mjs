...
const NON_EDITORIAL_TITLES = new Set([
  "home",
  "about us",
  "social feed",
  "blog",
...
const PLACEHOLDER_CONTENT = /(?:this is an example page|lorem ipsum|bookmarksgrove|all-powerful pointing|seven versalia|far world of grammar|woocommerce_checkout|\[the_ad\b)/i;
const AFRICA_CONTEXT = /(?:\bafrica\b|\bafrican\b|\bafro\b|\bafropolitan\b|\bdiaspora\b|\bafcfta\b|\becowas\b|\bafrican union\b|\bsahel\b|\bzambezi\b|\bvictoria falls\b|\bbrics\b)/i;

function isSearchableEditorialRecord(record, type) {
  // The public field is a story field. WordPress pages and plugin post types are
  // preserved in the raw export, but they must never compete in search results.
  if (type.slug !== "post") return false;
  ...
  if (!title || NON_EDITORIAL_TITLES.has(title)) return false;
  if (PLACEHOLDER_CONTENT.test(text)) return false;
  if (String(record.body || "").trim().length < 80) return false;

  // AFRO is an African editorial field, not a generic imported travel/theme
  // archive. Keep stories grounded in an African country or explicit African
  // continental/diaspora context.
  if (!(record.countries || []).length && !AFRICA_CONTEXT.test(text)) return false;
  ...
  for (const type of selectedTypes) {
    try {
      ...
      for (const row of rows) {
        const record = normalizePost(row, type, mediaMap);
        if (isSearchableEditorialRecord(record, type)) records.push(record);
      }
    } catch (error) {
